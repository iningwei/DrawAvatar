...
namespace ZGame.UGUIExtention
{
    ...
    public class ScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup
    {
        ...
        void ICanvasElement.LayoutComplete()
        {
            ...
            {
                //Debug.Log("RebuildList from ICanvasElement.LayoutComplete");
        ...
        protected void LateUpdate()
        {
            ...
            if (childItemRenderSiblingType != 0)
            {
                SetChildRenderSibling();
            }
        }

        [Tooltip("子物体渲染层级类型：0, 不做特殊处理；1，顺序，即按照子物体对应的index依次设置SiblingIndex；2，逆序，即index越大的，SiblingIndex越小")]
        public int childItemRenderSiblingType = 0;
        List<RectTransform> childs = new List<RectTransform>();
        private void SetChildRenderSibling()
        {
            childs.Clear();
            //设置子物体渲染层级
            for (int i = 0; i < this.content.childCount; i++)
            {
                var child = this.content.GetChild(i) as RectTransform;
                if (int.TryParse(child.name, out int x))
                {
                    childs.Add(child);
                }
            }
            if (childItemRenderSiblingType == 1)
            {
                childs.Sort((a, b) => a.name.CompareTo(b.name));
            }
            else if (childItemRenderSiblingType == 2)
            {
                childs.Sort((a, b) => b.name.CompareTo(a.name));
            }


            for (int j = 0; j < childs.Count; j++)
            {
                childs[j].SetSiblingIndex(j);
            }
        }
        #endregion
        ...
        private void UpdatePosition(bool forceRender = false)
        {
            ...
            for (int i = m_StartIdx; i < m_EndIdx;)
            {
                for (int j = 0; j < m_FixedCount; j++)
                {
                    ...
                    else
                    {
                        ...
                        //粗暴一点，直接设置为true，此bug彻底修复比较复杂耗时
                        //以下为限定bug：
                        //当数据行数==UI列表区域所能同时显示的最大行数+1时，把列表拖动到最底部，然后在拖到最顶部，此时会出现顶部一行虚拟列表没有调用 onItemRender 回调
                        //item.renderable = true;
                        //貌似会产生新问题，onItemRender回调执行暴增，用下方的解决方式
                    }
                }
                line++;
                i += (int)m_FixedCount;
            ...
            {
                var item = m_VirtualItems[i];
                bool update = item.UpdateActive();
                //峰瑜：添加一个update状态，修复上方所说的bug
                // Render在Active后
                if (update || item.renderable)
                {
#if UNITY_EDITOR
                    Debug.Log("item render:" + item.actualIndex);
#endif
        ...
        private Vector2 CalculateClosestPosition(int target)
        {
            ...
            else
            {
                if (m_MotionType == MotionType.Horizontal)
                {
                    ...
                    //    ? GetClosestLoopIndex(target)
                    //    : GetLoopIndex(-GetCellSize(0) * 0.5f);
                    offset.x = -(margin.left + itemSize.x * target + (target - 1) * m_Space.x);
                    //为结尾的数据特殊处理，避免最后几条数据的定位出现回弹影响视觉表现
                    //int endIndex = (int)(this.viewport.rect.width / (this.itemSize.x + m_Space.x)) + 1;
                    //if (target >= this.m_NumItems - endIndex)
                    //{
                    //    offset.x += (endIndex - (this.numItems - target)) * (this.itemSize.x + m_Space.x);
                    //}

                    //宠物皮肤列表临时方案
                    //前后填充空数据，做到目标数据居中，以及真实数据过少的情况下也能滚动到中间
                    offset.x += this.viewport.rect.width / 2 - itemSize.x * 0.5f - m_Space.x;
                }
                else
                {
                    offset.y = margin.top + itemSize.y * target + (target - 1) * m_Space.y;
                }
            }
            return offset;
        }
