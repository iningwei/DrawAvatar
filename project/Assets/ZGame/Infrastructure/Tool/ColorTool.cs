...
namespace ZGame
{
    public static class ColorTool
    {
        ...
        //本算法特点
        //1，排除绿色、红色、黑色、灰色色域
        //2, 保证了唯一性，uid和color的唯一映射 
        public static Color GenerateUniqueColorFromUid(string uid)
        {
            // 获取 Uid 的哈希值
            ...
            // 定义颜色范围（0-1 范围，排除绿色、红色、黑色、灰色）
            // 例如：R: 0.2-0.8, G: 0.2-0.6, B: 0.4-1.0
            float r = 0.2f + ((positiveHash % 100) / 100f) * 0.6f; // 0.2 到 0.8
            float g = 0.2f + (((positiveHash / 100) % 100) / 100f) * 0.4f; // 0.2 到 0.6
            float b = 0.4f + (((positiveHash / 10000) % 100) / 100f) * 0.6f; // 0.4 到 1.0

            // 确保颜色符合要求
            if (r > 0.7f && g < 0.3f && b < 0.3f) // 避免红色
                r = 0.5f; // 调整 R
            if (g > 0.5f && r < 0.3f && b < 0.3f) // 避免绿色
                g = 0.4f; // 调整 G
            if (r < 0.3f && g < 0.3f && b < 0.3f) // 避免黑色
                b = 0.7f; // 提高 B
            if (Mathf.Abs(r - g) < 0.1f && Mathf.Abs(g - b) < 0.1f) // 避免灰色
                b += 0.2f; // 增加 B 的差异
